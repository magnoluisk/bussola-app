...
import { Check, X, TrendingUp, TrendingDown, ChevronRight, Globe2, Shield, Zap, Scale, HelpCircle, ArrowRight, ShoppingCart, ExternalLink, Layers, PiggyBank, Bitcoin, AlertTriangle, FileText, BookOpen, Brain, Receipt, AlertCircle, Mail, RefreshCw, Wallet, CreditCard, Umbrella, Mic, MicOff, Trash2, Compass } from "lucide-react";
...
export default function BussolaEducacaoDeInvestimentos() {
  ...
  const [custosFixos, setCustosFixos] = useState([
    { id: 1, nome: "Aluguel/Financiamento", valor: 1200, diaVencimento: 5 },
    { id: 2, nome: "Contas (luz, água, internet)", valor: 350, diaVencimento: 10 },
    { id: 3, nome: "Transporte", valor: 300, diaVencimento: 15 },
  ]);
  ...
  return (
    ...
    >
      ...
      {/* Abas de classe de ativo */}
      <div className="px-5 md:px-10 pt-6 flex gap-2 overflow-x-auto" style={{ whiteSpace: "nowrap" }}>
        {[
          { key: "vida-financeira", label: "Vida Financeira" },
          { key: "carteira", label: "Minha Carteira" },
          { key: "relatorio", label: "Relatório Periódico" },
          { key: "diario", label: "Diário do Investidor" },
          { key: "custos", label: "Custos Ocultos" },
          ...
          <button
            ...
            className="px-4 py-2 text-sm font-semibold rounded-sm transition-colors shrink-0"
            style={{
              background: assetClass === tab.key ? "var(--panel)" : "transparent",
              color: assetClass === tab.key ? "var(--gold)" : "var(--paper-dim)",
              borderBottom: `2px solid ${assetClass === tab.key ? "var(--gold)" : "transparent"}`,
            }}
          >
            {tab.label}
          </button>
          ...
          {custosFixos.some((c) => statusVencimento(c.diaVencimento)?.urgente) && (
            <div className="flex items-start gap-2 mb-2 p-2.5 rounded-sm" style={{ background: "rgba(177,74,52,0.1)", border: "1px solid rgba(177,74,52,0.35)" }}>
              ...
              <p className="text-xs leading-relaxed" style={{ color: "var(--paper)" }}>
                Você tem conta{custosFixos.filter((c) => statusVencimento(c.diaVencimento)?.urgente).length > 1 ? "s" : ""} vencida ou vencendo nos próximos dias — confira abaixo.
              </p>
            </div>
          )}
              ...
              return (
                <div key={c.id} className="flex items-center justify-between text-xs px-3 py-1.5 rounded-sm" style={{ border: `1px solid ${status?.urgente ? "rgba(177,74,52,0.35)" : "rgba(237,230,214,0.12)"}` }}>
                  <div>
                    ...
                    {status && (
                      <span className="ml-2 px-1.5 py-0.5 rounded-sm text-[10px]" style={{ border: `1px solid ${status.color}`, color: status.color }}>
                        {status.label}
                      </span>
                    )}
                  </div>
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={() => {
                ...
                setCustosFixos((prev) => [...prev, { id: Date.now(), nome: novoCustoFixo.nome, valor: Number(novoCustoFixo.valor), diaVencimento: novoCustoFixo.diaVencimento ? Number(novoCustoFixo.diaVencimento) : null }]);
        ...
        <div className="rounded-sm p-4 md:p-5 mt-5" style={{ background: "var(--panel)", border: "1px solid rgba(237,230,214,0.15)" }}>
          ...
          {investimentos.length > 0 && (
            <div className="mt-4 pt-3 flex items-center justify-between text-sm" style={{ borderTop: "1px solid rgba(237,230,214,0.15)" }}>
              <span style={{ color: "var(--paper-dim)" }}>Total já investido</span>

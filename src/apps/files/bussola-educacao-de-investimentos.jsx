...
import { Check, X, TrendingUp, TrendingDown, ChevronRight, Globe2, Shield, Zap, Scale, HelpCircle, ArrowRight, ShoppingCart, ExternalLink, Layers, PiggyBank, Bitcoin, AlertTriangle, FileText, BookOpen, Brain, Receipt, AlertCircle, Mail, RefreshCw, Wallet, CreditCard, Umbrella, Mic, MicOff, Trash2, Compass, PieChart, Landmark, Building2, Eye, EyeOff, Link2, Pencil, Search, Headphones, Volume2 } from "lucide-react";
...
// ---------- Bússola da Prosperidade: ideias de renda extra calibradas pelo perfil ----------
const INCOME_IDEAS = {
  conservador: [
...
function IconMenuSection({ title, items, assetClass, setAssetClass }) {
  return (
...
export default function BussolaEducacaoDeInvestimentos() {
  ...
  const totalDisponivel = contaCorrente + saldoEspecie;
  const [showSaldo, setShowSaldo] = useState(true);
  const [rendaFixa, setRendaFixa] = useState(3500);
  const [rendaVariavel, setRendaVariavel] = useState(700);
  const [rendaExtra, setRendaExtra] = useState(300);
  ...
  function startVoiceCapture() {
    ...
    recognition.onresult = (event) => {
      ...
      setVoicePreview(parseVoiceExpense(transcript));
    ...
    >
      ...
      <header className="px-5 pt-8 pb-6 md:px-10" style={{ borderBottom: "1px solid rgba(237,230,214,0.15)" }}>
        ...
        <h1 className="text-3xl md:text-4xl" style={{ fontFamily: "'Roboto Slab', serif", fontWeight: 700 }}>
          Bússola Educação de Investimentos
        </h1>
      ...
      {/* Menu de navegação em grid de ícones, estilo "Favoritos" de app de banco */}
      <IconMenuSection
        title="Meu Patrimônio"
        items={[{ key: "vida-financeira", label: "Minha Vida Financeira", icon: Wallet }]}
      ...
      <div className="px-5 py-6 md:px-10 max-w-3xl">
        ...
        {/* Saldo em conta corrente + em espécie */}
        <div className="rounded-sm p-4 md:p-5 mb-5" style={{ background: "var(--panel)", border: "1px solid rgba(237,230,214,0.15)" }}>
          ...
          <div className="flex items-center gap-1.5 mt-3 text-[11px]" style={{ color: "var(--paper-dim)" }}>
            <Link2 size={12} />
            Conta corrente preenchida manualmente por enquanto — quando você conectar seu banco (Open Finance), atualiza sozinha. O valor em espécie sempre será manual, já que dinheiro físico não tem como se conectar a nada.
          </div>
        </div>
        ...
        >
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-sm font-bold" style={{ color: "var(--gold)", fontFamily: "'Roboto Slab', serif" }}>Meu Consultor Rumo</span>
          </div>
          ...
          >
            ...
            {isListeningConsultor ? "Ouvindo... fale sua pergunta" : "Falar com o Rumo"}
          </button>
          ...
          <p className="mt-3 text-[10px] leading-relaxed" style={{ color: "var(--paper-dim)" }}>
            O Rumo responde com base nos dados que você já cadastrou aqui no app — hoje ele reconhece perguntas sobre orçamento do dia a dia, dívidas, reserva de emergência e saldo. A intenção do Rumo é te orientar no seu dia a dia, pra evitar gastos impulsivos e desnecessários. Se a resposta em áudio não tocar automaticamente, seu navegador pode não suportar leitura de voz.
          </p>
        </div>

        {/* Diagnóstico financeiro */}
        <div className="rounded-sm p-4 md:p-5 mb-5" style={{ background: "var(--panel)", border: "1px solid rgba(237,230,214,0.15)" }}>
          <div className="text-xs font-semibold uppercase tracking-wide mb-3" style={{ color: "var(--gold)" }}>1. Diagnóstico financeiro</div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-2">
            <div>
              ...
              <input
                type="number" min="0" value={rendaFixa}
                onChange={(e) => setRendaFixa(Math.max(0, Number(e.target.value)))}
            ...
            <div>
              <label className="text-[10px] block mb-1" style={{ color: "var(--paper-dim)" }}>Renda variável (comissão, bônus)</label>
              <input
                type="number" min="0" value={rendaVariavel}
                onChange={(e) => setRendaVariavel(Math.max(0, Number(e.target.value)))}
                className="w-full text-sm px-2 py-1.5 rounded-sm"
                style={{ background: "var(--ink)", border: "1px solid rgba(237,230,214,0.25)", color: "var(--paper)", fontFamily: "'JetBrains Mono', monospace" }}
              />
            </div>
            <div>
              <label className="text-[10px] block mb-1" style={{ color: "var(--paper-dim)" }}>Renda extra (freela, bico)</label>
              <input
                type="number" min="0" value={rendaExtra}
                onChange={(e) => setRendaExtra(Math.max(0, Number(e.target.value)))}
                className="w-full text-sm px-2 py-1.5 rounded-sm"
                style={{ background: "var(--ink)", border: "1px solid rgba(237,230,214,0.25)", color: "var(--paper)", fontFamily: "'JetBrains Mono', monospace" }}
              />
            </div>
          ...
          <div className="space-y-1.5 mb-2">
            {custosFixos.map((c) => {
              ...
              return (
                <div key={c.id} className="flex items-center justify-between text-xs px-3 py-1.5 rounded-sm" style={{ border: `1px solid ${urgente ? "rgba(177,74,52,0.35)" : "rgba(237,230,214,0.12)"}`, opacity: c.pago ? 0.6 : 1 }}>
                  <div className="flex items-center gap-2">
                    <span style={{ color: "var(--paper)", textDecoration: c.pago ? "line-through" : "none" }}>{c.nome}</span>
                    {c.pago ? (
                      <span className="px-1.5 py-0.5 rounded-sm text-[10px]" style={{ border: "1px solid var(--gold)", color: "var(--gold)" }}>Pago</span>
                    ) : status && (
                      <span className="px-1.5 py-0.5 rounded-sm text-[10px]" style={{ border: `1px solid ${status.color}`, color: status.color }}>
                        {status.label}
                      </span>
                    )}
                  </div>
                    ...
                    {editingCustoId === c.id ? (
                      <>
                        <input
                          ...
                          className="w-20 text-xs px-1.5 py-1 rounded-sm"
          ...
          <div className="flex gap-2">
            ...
            />
            <input
              type="number" min="0" placeholder="R$" value={novoCustoFixo.valor}
          ...
          <p className="text-[10px] mt-1.5" style={{ color: "var(--paper-dim)" }}>
            Por áudio, fale algo como "aluguel 600 reais, vence dia 5" — a gente já tenta preencher nome, valor e dia sozinho, pra você só conferir antes de adicionar.
          </p>

          <div className="grid grid-cols-3 gap-2 mt-4 pt-4" style={{ borderTop: "1px solid rgba(237,230,214,0.15)" }}>
            <div>
              <div className="text-[10px] uppercase" style={{ color: "var(--paper-dim)" }}>Renda</div>
              <div className="text-sm font-semibold" style={{ color: "var(--paper)", fontFamily: "'JetBrains Mono', monospace" }}>R$ {rendaMensal.toLocaleString("pt-BR")}</div>
            </div>
            <div>
              <div className="text-[10px] uppercase" style={{ color: "var(--paper-dim)" }}>Custos fixos</div>
              <div className="text-sm font-semibold" style={{ color: "var(--rust)", fontFamily: "'JetBrains Mono', monospace" }}>R$ {totalCustosFixos.toLocaleString("pt-BR")}</div>
            </div>
            <div>
              <div className="text-[10px] uppercase" style={{ color: "var(--paper-dim)" }}>Sobra mensal</div>
              <div className="text-sm font-semibold" style={{ color: sobraMensal >= 0 ? "var(--gold)" : "var(--rust)", fontFamily: "'JetBrains Mono', monospace" }}>R$ {sobraMensal.toLocaleString("pt-BR")}</div>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-2 mt-3 pt-3" style={{ borderTop: "1px solid rgba(237,230,214,0.15)" }}>
            <div>
              <div className="text-[10px] uppercase" style={{ color: "var(--paper-dim)" }}>Custo Variável (ex: mercado, transporte, lazer)</div>
            ...
            <div>
              <div className="text-[10px] uppercase" style={{ color: "var(--paper-dim)" }}>Custos Extras (ex: conserto, presente, emergência)</div>
          ...
          {/* Botão sempre visível pra abrir a Bússola da Prosperidade manualmente */}
          <button
            onClick={() => setShowProsperity((v) => !v)}
            className="flex items-center gap-2 text-sm font-semibold mt-4 px-4 py-2.5 rounded-sm transition-colors"
            style={{ background: "var(--gold)", color: "var(--ink)" }}
          >
            <Compass size={16} />
            {prosperityVisible ? "Ocultar Bússola da Prosperidade" : "Ver Bússola da Prosperidade — ideias de renda extra"}
            <ArrowRight size={14} style={{ transform: prosperityVisible ? "rotate(90deg)" : "none", transition: "transform 0.15s" }} />
          </button>
        </div>

        {/* Bússola da Prosperidade — sempre acessível pelo botão acima; abre automaticamente quando a sobra é curta ou negativa */}
        {prosperityVisible && (
          <div className="rounded-sm p-4 md:p-5 mb-5" style={{ background: "rgba(190,154,92,0.08)", border: "1px solid rgba(190,154,92,0.3)" }}>
            <div className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide mb-2" style={{ color: "var(--gold)" }}>
              <Compass size={14} /> Sua Bússola da Prosperidade
            </div>
        ...
        {/* Orçamento diário + registro de gastos (com voz) */}
        <div className="rounded-sm p-4 md:p-5 mb-5" style={{ background: "var(--panel)", border: "1px solid rgba(237,230,214,0.15)" }}>
          <div className="text-xs font-semibold uppercase tracking-wide mb-3" style={{ color: "var(--gold)" }}>2. Orçamento diário disponível</div>
          ...
          <div className="grid grid-cols-2 gap-2 mb-2">
            <div className="p-2 rounded-sm" style={{ border: "1px solid rgba(237,230,214,0.15)" }}>
              ...
              <p className="text-[10px] leading-relaxed" style={{ color: "var(--paper-dim)" }}>Gastos do dia a dia que mudam de mês pra mês, mas você já espera que vão acontecer — mercado, transporte, lazer.</p>
            </div>
            <div className="p-2 rounded-sm" style={{ border: "1px solid rgba(237,230,214,0.15)" }}>
          ...
          <div className="flex gap-2 mb-2">
            {[{ key: "variavel", label: "Variável" }, { key: "extra", label: "Extra" }].map((t) => (
              <button
                key={t.key}
                onClick={() => setNovoGasto((f) => ({ ...f, tipo: t.key }))}
                className="px-3 py-1 text-xs rounded-sm"
                style={{
                  background: novoGasto.tipo === t.key ? "var(--gold)" : "transparent",
                  color: novoGasto.tipo === t.key ? "var(--ink)" : "var(--paper-dim)",
                  border: `1px solid ${novoGasto.tipo === t.key ? "var(--gold)" : "rgba(237,230,214,0.25)"}`,
                }}
              >
                {t.label}
              </button>
          ...
          <div className="flex gap-2 mb-2">
            ...
            />
            <input
              type="number" min="0" placeholder="R$" value={novoGasto.valor}
            ...
            <button
              onClick={() => {
                ...
                setGastosVariaveis((prev) => [{ id: Date.now(), nome: novoGasto.nome, valor: Number(novoGasto.valor), tipo: novoGasto.tipo }, ...prev]);
          ...
          {!voiceSupported && (
            <p className="text-[10px] mb-2" style={{ color: "var(--paper-dim)" }}>Reconhecimento de voz não suportado neste navegador — use o campo de texto acima.</p>
          )}
          ...
          {voicePreview && (
            <div className="mb-3 p-3 rounded-sm" style={{ background: "rgba(190,154,92,0.1)", border: "1px solid rgba(190,154,92,0.3)" }}>
              <div className="text-xs mb-2" style={{ color: "var(--paper)" }}>
                Entendi: <strong>"{voicePreview.nome}"</strong> — categoria {EXPENSE_CATEGORIES.find((c) => c.key === voicePreview.categoria)?.label}
                {voicePreview.valor ? ` · R$ ${voicePreview.valor.toLocaleString("pt-BR")}` : " · não identifiquei o valor, confirme abaixo"}
              </div>
              <div className="flex gap-2 mb-2">
                {[{ key: "variavel", label: "Variável" }, { key: "extra", label: "Extra" }].map((t) => (
                  <button
                    key={t.key}
                    onClick={() => setVoicePreview((v) => ({ ...v, tipo: t.key }))}
                    className="px-3 py-1 text-xs rounded-sm"
                    style={{
                      background: voicePreview.tipo === t.key ? "var(--gold)" : "transparent",
                      color: voicePreview.tipo === t.key ? "var(--ink)" : "var(--paper-dim)",
                      border: `1px solid ${voicePreview.tipo === t.key ? "var(--gold)" : "rgba(237,230,214,0.25)"}`,
                    }}
                  >
                    {t.label}
                  </button>
              ...
              <div className="flex gap-2">
                <button
                  onClick={() => {
                    if (voicePreview.valor) {
                      setGastosVariaveis((prev) => [{ id: Date.now(), nome: voicePreview.nome, valor: voicePreview.valor, tipo: voicePreview.tipo || "variavel" }, ...prev]);
                    }
                    setVoicePreview(null);
                  }}
                  className="text-xs font-semibold px-3 py-1 rounded-sm" style={{ background: "var(--gold)", color: "var(--ink)" }}
                >
                  Confirmar registro
                </button>
                <button onClick={() => setVoicePreview(null)} className="text-xs px-3 py-1 rounded-sm" style={{ border: "1px solid rgba(237,230,214,0.25)", color: "var(--paper-dim)" }}>
                  Descartar
                </button>
              </div>
            </div>
                ...
                {gastosVariaveis.map((g) => (
                  <div key={g.id} className="flex items-center justify-between text-xs px-3 py-1.5 rounded-sm" style={{ border: "1px solid rgba(237,230,214,0.12)" }}>
                    <span style={{ color: "var(--paper)" }}>
                      {g.nome}{" "}
                      <span
                        className="ml-1 px-1.5 py-0.5 rounded-sm text-[10px]"
                        style={{ border: `1px solid ${g.tipo === "extra" ? "rgba(177,74,52,0.4)" : "rgba(190,154,92,0.4)"}`, color: g.tipo === "extra" ? "var(--rust)" : "var(--gold)" }}
                      >
          ...
          <p className="mt-3 text-[10px] leading-relaxed" style={{ color: "var(--paper-dim)" }}>
            Voz e registro manual funcionam nesta versão. Detectar gastos automaticamente direto do seu banco/cartão é viável numa versão conectada via Open Finance (que já compartilha dados de conta corrente e cartões) — diferente da execução de compra de ativos, essa parte do Open Finance já é madura hoje.
          </p>
        </div>
        ...
        <div className="rounded-sm p-4 md:p-5 mb-5" style={{ background: "var(--panel)", border: "1px solid rgba(237,230,214,0.15)" }}>
          <div className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide mb-3" style={{ color: "var(--gold)" }}>
            <CreditCard size={13} /> 3. Quitação de dívidas
          </div>
        ...
        <div className="rounded-sm p-4 md:p-5" style={{ background: "var(--panel)", border: "1px solid rgba(237,230,214,0.15)" }}>
          <div className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide mb-1" style={{ color: "var(--gold)" }}>
            <Umbrella size={13} /> 4. Fundo de emergência guiado
          </div>
          <button
          ...
          <input
            ...
            className="w-40 text-sm px-2 py-1.5 rounded-sm mb-4"
            style={{ background: "var(--ink)", border: "1px solid rgba(237,230,214,0.25)", color: "var(--paper)", fontFamily: "'JetBrains Mono', monospace" }}
          />
        ...
        {/* Fundo de investimento — o que a pessoa já tem investido, ativo por ativo */}
        <div className="rounded-sm p-4 md:p-5 mt-5" style={{ background: "var(--panel)", border: "1px solid rgba(237,230,214,0.15)" }}>
          <div className="flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wide mb-3" style={{ color: "var(--gold)" }}>
            <Wallet size={13} /> 5. Fundo de investimento
          </div>
          <p className="text-xs mb-3" style={{ color: "var(--paper-dim)" }}>
          ...
          <div className="flex gap-2">
            ...
            />
            <input
              type="number" min="0" placeholder="Valor investido" value={novoInvestimento.valor}
              onChange={(e) => setNovoInvestimento((f) => ({ ...f, valor: e.target.value }))}
              className="w-32 text-xs px-2 py-1.5 rounded-sm"
              style={{ background: "var(--ink)", border: "1px solid rgba(237,230,214,0.25)", color: "var(--paper)", fontFamily: "'JetBrains Mono', monospace" }}
            />
            <button
              onClick={() => {
          ...
          <div className="flex items-center justify-between flex-wrap gap-2 pb-4" style={{ borderBottom: "1px solid rgba(237,230,214,0.15)" }}>
            <div>
              <div className="text-xs uppercase tracking-widest" style={{ color: "var(--gold)", fontFamily: "'JetBrains Mono', monospace" }}>
                Bússola Educação de Investimentos · Relatório {reportPeriod === "quinzenal" ? "Quinzenal" : "Mensal"}
              </div>
      ...
      <div className="px-5 py-6 md:px-10 max-w-3xl">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-6">
          {[
            { key: "valorInicial", label: "Valor inicial (R$)" },
            { key: "aporteMensal", label: "Aporte mensal (R$)" },
            { key: "anos", label: "Anos" },
            ...
            {showBrokers && (
              ...
              >
                ...
                <div className="mt-3 pt-3" style={{ borderTop: "1px solid rgba(237,230,214,0.15)" }}>
                  ...
                  <div className="flex gap-2">
                    <input
                      type="number" min="0" placeholder="Valor aplicado (R$)" value={confirmValor}
